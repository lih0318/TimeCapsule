...
{
    Heap *NewHeap = (Heap *)malloc(sizeof(Heap));                              // 새로운 힙 영역 생성
    NewHeap->Capacity = InitialSize;                                           // 초기 사이즈
    NewHeap->UsedSize = 0;                                                     // 사용된 영역
...
void HEAP_Insert(Heap *H, ElementType NewData)
{
    ...
    if (H->UsedSize == H->Capacity) // UsedSize가 Capacity에 도달하면 2배로 늘림
    {
        H->Capacity *= 2;
...
void HEAP_DeleteMin(Heap *H, HeapNode *Root)
{
    ...
    H->UsedSize--;
    HEAP_SwapNodes(H, 0, H->UsedSize); // 최하 최우측 노드를 Root로 이동
